  ...
  notFoundComponent: () => (
    <div className="grid min-h-screen place-items-center px-6 text-center">
      <div>
        ...
        <Link to="/projects" className="mt-6 inline-block text-accent">Back to projects</Link>
      </div>
    </div>
  ),
    ...
    <>
      ...
      <section ref={heroRef} className="relative -mt-12 overflow-hidden pt-20 pb-24 sm:pt-32">
        <motion.div
          ...
          className={cn("absolute inset-0 -z-10 bg-gradient-to-b", project.accent)}
        />
        ...
        <div className="mx-auto max-w-5xl px-6">
          ...
          <div className="mt-10 flex flex-col gap-8 sm:flex-row sm:items-end sm:gap-10">
            <Reveal>
              <img src={project.icon} alt="" width={128} height={128} className="h-28 w-28 rounded-3xl shadow-card sm:h-36 sm:w-36" />
            </Reveal>
            <div className="flex-1">
              <Reveal delay={0.05}>
                <p className="text-eyebrow text-accent">{project.category} · {project.year}</p>
              </Reveal>
              <Reveal delay={0.1}>
                <h1 className="mt-3 text-display-xl">{project.name}</h1>
              </Reveal>
              <Reveal delay={0.15}>
      ...
      <section className="mx-auto max-w-5xl px-6 py-16">
        <Reveal>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {project.stack.map((t) => (
              <span key={t} className="rounded-full border border-border bg-card px-4 py-1.5 text-sm">{t}</span>
            ))}
          </div>
        </Reveal>
      </section>
        ...
        <div className="mx-auto max-w-5xl px-6">
          <div className="flex items-end justify-between">
            ...
            <div className="hidden gap-2 sm:flex">
              <button onClick={() => scrollSlider(-1)} className="grid h-10 w-10 place-items-center rounded-full border border-border bg-card transition-colors hover:bg-muted">
